...
GEOMETRY_TYPES = ('linestring', 'point', 'polygon', 'geometry',
				'multipolint', 'multilinestring', 'multipolygon',
				'geometrycollection',)
...
def convert_data(col, data):
	...
	if col.type in GEOMETRY_TYPES:
		return GeometryText(data)
	if data is None and col.is_nullable == False:
		if convert_type(col.type) == 'timestamp':
			return '1970-01-01 00:00:00'
		if convert_type(col.type) == 'date':
			return '1970-01-01'
	return data
...
def main ():
	...
	mysql_cur = mysql_conn.cursor(cursorclass=SSDictCursor)
	...
	for table in tables:
		...
		ins_sql = ('INSERT INTO "%s".%s (%s) VALUES (%s);' %
				   (schema, pg_table,
					', '.join('"%s"' % fix_reserved_word(c.name) for c in cols),
					','.join(['%s'] * len(cols))))
		...
		while True:
			...
			for c in cols:
				data = row[c.name]
				output_L.append(convert_data(c, data))
